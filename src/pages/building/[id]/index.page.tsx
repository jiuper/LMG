...
export default function LiftMediaPage({ port, id }: InferGetStaticPropsType<typeof getStaticProps>) {
    const items = [{ label: "Реклама в жилых домах", url: Routes.BUILDING }, { label: filterCategories[+id] }];

    return (
        <PageLayout>
            <BreadCrumb model={items} />
            <LiftMedia
                caption={filterCategories[+id]}
...
export const getStaticPaths = () => {
    const categories = Array(filterCategories.length)
        .fill(0)
        .map((_, i) => i.toString());
