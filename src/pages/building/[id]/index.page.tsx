...
export default function LiftMediaPage({ port, id, cat, area }: InferGetServerSidePropsType<typeof getServerSideProps>) {
    ...
    const items = [{ label: "Реклама в жилых домах", url: Routes.BUILDING }, { label: filter.title }];
    const filterPort = port.filter((el) => el.categoryId === filter.id);

    return (
        <PageLayout>
            <BreadCrumb model={items} />
            <LiftMedia url={`${Routes.BUILDING}/${id}`} data={filter} port={filterPort} districts={area} />
        </PageLayout>
    );
}
